...
public class ExportMyRequestsModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        ws.Cell(1, 7).Value = "Proyecto";
        ws.Cell(1, 8).Value = "Responsable";
        ws.Cell(1, 9).Value = "Cliente (HW)";
        ws.Cell(1, 10).Value = "Serie";
        ...
        {
            ws.Cell(r, 1).Value = m.RequestedAt.ToLocalTime().ToString("yyyy-MM-dd HH:mm");
            ws.Cell(r, 2).Value = m.Type == InventoryMovementType.In ? "Entrada" : "Salida";
            ws.Cell(r, 3).Value = m.Item?.Name;
            ws.Cell(r, 4).Value = m.Item?.Sku;
            ws.Cell(r, 5).Value = m.Quantity;
            ws.Cell(r, 6).Value = m.Item?.Unit;
            ws.Cell(r, 7).Value = m.Project?.Title;
            ws.Cell(r, 8).Value = m.ResponsibleName;
            ws.Cell(r, 9).Value = m.AssignedClient?.Name;
            ws.Cell(r, 10).Value = m.SerialNumber;
            ws.Cell(r, 11).Value = m.Status.ToString();
